...
def variable_neighborhood_search(solution, makespan, processing_time):
    ...
    while evaluation_times < 2000000:
        previous_makespan = local_makespan
        if k == 1:
        ...
        while True:
            ...
            if one_step_min_makespan < local_makespan:
                ...
                break
        if local_makespan < previous_makespan:
            if local_makespan < makespan:
                makespan = local_makespan
                solution = local_solution.copy()
            counter = 0
        else:
            counter += 1
            if counter == 3:
                ...
                while True:
                    ...
                    new_solution, new_makespan = neighborhood((i1, i2, local_solution, processing_time))
                    if new_makespan < local_makespan + 200:
                        local_solution = new_solution
                        local_makespan = new_makespan
                        break
        k += 1
        if k > 3:
            k = 1
